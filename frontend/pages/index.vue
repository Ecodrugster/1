    ...
    <main class="md:col-span-6 space-y-6">
      ...
      <div class="bg-slate-900 border border-white/5 rounded-xl p-6 shadow-xl">
        <div class="flex items-start space-x-4">
          <div class="w-10 h-10 rounded-full bg-gradient-to-tr from-blue-500 to-purple-500 flex-shrink-0"></div>
          <div class="flex-grow">
            <textarea 
              v-model="newPost"
      ...
      <div class="space-y-6">
        ...
        <div v-for="post in posts" :key="post.id" class="bg-slate-900 border border-white/5 rounded-xl p-6 shadow-xl hover:border-blue-500/20 transition-all">
          <div class="flex items-center space-x-4 mb-4">
            ...
            <div>
              <div class="text-white font-semibold text-sm">Студент #{{ post.author_id.substring(0, 4) }}</div>
              <div class="text-slate-500 text-[10px] uppercase">{{ new Date(post.created_at).toLocaleString('ru-RU') }}</div>
            </div>
          </div>
          ...
          <div v-if="post.showComments" class="mt-4 pt-4 border-t border-white/5 space-y-4">
            <div v-for="comment in post.comments" :key="comment.id" class="flex items-start space-x-3">
              ...
              <div class="bg-slate-800/50 rounded-lg p-3 flex-grow">
                <div class="text-xs font-semibold text-white mb-1">Студент #{{ comment.author_id.substring(0, 4) }}</div>
...
const likePost = async (post) => {
  if (post.liked) return // Simple client-side prevention of double likes for now
